...
public class Shelf {
    ...
    public static boolean addBook(int id, String author, String title, int pages ){
        if((isIDUnique(id) == true) && !title.isEmpty() && pages > 0){
            theBooks.add(new Book(id,author,title,pages));
            return true;
        }
        return false;
    ...
    public static boolean addBook(Book book ){
        if((isIDUnique(book.getId()) == true) && !book.getTitle().isEmpty() && (book.getPages() > 0)){
            theBooks.add(book);
            return true;
        }
    ...
    public static void removeBook(String author) {
        for(int i = 0; i < theBooks.size(); i++){
            if(author.equalsIgnoreCase(theBooks.get(i).getAuthor())){
                theBooks.remove(i);
                i--;
            }
        }
