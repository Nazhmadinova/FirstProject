...
public class Book {
    ...
    public void setAuthor(String author) {
        if(author.length() == 0){
            ...
            if(author.trim().contains(" ")){
                String [] ar=author.trim().split(" ");
                String str="";
                for(int i=0; i< ar.length; i++){
                    str=str+capitalize(ar[i]) +" ";
                }
    ...
    public void setTitle(String title) {
        if(!title.isEmpty() && !title.trim().contains(" ")){
            this.title=capitalize(title);
        }else if(title.isEmpty()){
            this.title="";
        }else if(!title.isEmpty() && title.trim().contains(" ")){
            String [] arr= title.trim().split(" ");
            String str="";
            for(int i = 0; i < arr.length; i++){
                if(i != 0 && isTitleWord(arr[i])==false){
                    str=str+arr[i].toLowerCase()+" ";
                }else{
                    str=str+capitalize(arr[i])+" ";
                }
            }
            this.title=str.trim();
        }
    ...
    public String toString() {
        String str="Book[author="+this.author+", title="+this.title+", id="+this.id+", pages="+this.pages+"]";
        return str;
